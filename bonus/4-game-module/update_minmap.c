...
void	update_minmap_2(t_data *data, t_vector r)
{
	data->player->minmap_pos.y = MINMAP_HEIGHT * TILE_HEIGTH * MINMAP_FACTOR
		/ 2;
...
void	update_minmap(t_data *data)
{
	...
	while (++i.y < p.y + (MINMAP_HEIGHT / 2))
	{
		...
		{
			if (0 <= i.y && i.y < data->map_height && 0 <= i.x
				&& i.x < data->map_width)
				data->mini_map[i.y - (p.y - MINMAP_HEIGHT / 2)][i.x - (p.x
						- MINMAP_WIDTH / 2)] = data->map[i.y][i.x];
			else
				data->mini_map[i.y - (p.y - MINMAP_HEIGHT / 2)][i.x - (p.x
						- MINMAP_WIDTH / 2)] = ' ';
		}
	}
	update_minmap_2(data, r);
}
