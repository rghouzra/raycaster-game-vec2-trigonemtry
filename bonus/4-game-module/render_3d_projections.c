...
/*   Updated: 2024/01/17 11:43:27 by yrhiba           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "header.h"

void	render_3d_projections_3(t_data *data, int i)
{
...
void	render_3d_projections_2(t_data *data, int i, int j)
{
	...
	data->rays[i].texture_x = (data->rays[i].texture_x
			* data->rays[i].tex->width) / (TILE_SIZE * 1.0f);
	j = data->rays[i].start_row - 1;
	while (++j < data->rays[i].end_row)
	{
		data->rays[i].texture_y = j + (data->rays[i].wall_height / 2)
			- (WIN_HEIGHT / 2);
		data->rays[i].texture_y = data->rays[i].texture_y
			* (data->rays[i].tex->height / (data->rays[i].wall_height * 1.0f));
		frame_pixel_put(data->frame, i, j, frame_get_color(data->rays[i].tex,
				data->rays[i].texture_x, data->rays[i].texture_y));
	}
}
...
void	render_3d_projections(t_data *data)
{
	...
	while (++i < WIN_WIDTH)
	{
		...
		data->rays[i].wall_height = (TILE_HEIGTH
				/ data->rays[i].correct_wall_dis) * ((WIN_WIDTH / 2) / tan(FOV
					/ 2));
		data->rays[i].start_row = round((WIN_HEIGHT / 2)
				- (data->rays[i].wall_height / 2));
		if (data->rays[i].start_row < 0)
			data->rays[i].start_row = 0;
		data->rays[i].end_row = round((WIN_HEIGHT / 2)
				+ (data->rays[i].wall_height / 2));
		render_3d_projections_3(data, i);
		render_3d_projections_2(data, i, j);
	}
}
