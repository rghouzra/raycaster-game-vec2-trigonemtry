...
void	render_footer(t_data *data)
{
	...
	n = ft_itoa(data->fps_counted);
	my_string_append(&data->fs, n);
	my_string_append(&data->fs, " )");
	mlx_string_put(data->mlx, data->win, WIN_WIDTH - 180, WIN_HEIGHT - 50,
		getcolor(0, 0, 0, 0), data->fs);
	pt = (data->lastframetime.tv_sec - data->stime.tv_sec) * 1000.0f
		+ (data->lastframetime.tv_usec - data->stime.tv_usec) / 1000.0f;
	if (pt >= 1e3)
	{
		data->fps_counted = data->frame_count;
		data->frame_count = 0;
		ft_memcpy(&(data->stime), &(data->lastframetime),
